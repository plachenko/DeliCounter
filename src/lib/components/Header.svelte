...
<div id="Header" class="flex overflow-hidden w-full relative h-[50px] bg-slate-400 p-2 gap-1">
	...
	{#if curOrderItem?.name}
		<div
			...
			class="flex-1 flex justify-center relative items-center bg-slate-300 rounded-md font-bold text-slate-500"
		>
		...
		>
			...
			{:else}
				<div
					in:fly={{ y: -10 }}
					out:fly={{ y: -10 }}
					class="left-0 pr-9 absolute p-1 w-full h-full"
				>
					<button
						onclick={() => {
					...
					>
						...
						<div class="flex h-full">
							...
							>
								<div
									class="bg-slate-200/30 rounded-l-md flex flex-col relative flex-1 w-full h-full"
								>
									{#if selectingLanguage}
